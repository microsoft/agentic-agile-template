...
# https://aider.chat/docs/usage/conventions.html — `CONVENTIONS.md` is the
# filename shown in the official aider documentation and example transcripts.
# Confirm the current recommendation in the official documentation before
# deploying, as aider's tooling evolves.
...
<!--
  ...
  problem it solves. Helps the AI understand the domain and make
  appropriate code decisions.
...
<!--
  Rules aider should follow when generating or modifying code.
...
<!--
  Branch strategy, PR requirements, and anything aider should know
  when helping with git or CI tasks.
-->
...
<!--
  ...
  Prevents conflicts when aider and other agents work in parallel across the codebase.
-->
...
<!--
  Aider generates git commit messages automatically after applying edits.
  Specify your commit message conventions here so aider's auto-commits
  match your project's history format.

  Aider supports conventional commits and will follow the format you specify.
  Example formats:
  - Conventional Commits: "feat: add user authentication"
  - Gitmoji: "✨ add user authentication"
  - Ticket-prefixed: "PROJ-123: add user authentication"
-->

- **Format:** [e.g., Conventional Commits — `feat:`, `fix:`, `docs:`, `refactor:`, `test:`, `chore:`]
- **Scope:** [e.g., include scope in parens when relevant — `feat(auth): add OAuth2 support`]
- **Body:** [e.g., include a body when the change is non-obvious; wrap at 72 characters]
- **Breaking changes:** [e.g., mark with `BREAKING CHANGE:` in the footer]
...
<!--
  Explicit anti-patterns. Aider will treat these as hard rules.
-->
